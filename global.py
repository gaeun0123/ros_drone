...
class MultiDroneNode:
    ...
    def takeoff(self, altitude=30.0):
        ...
        print("OFFBOARD mode로 변경되었습니다.")

        self.arm()
        # 실제로 사용할 수 있는 좌표값을 가져와 설정합니다.
        latitude = self.current_global_pose.latitude
        longitude = self.current_global_pose.longitude

        self.takeoff_client(min_pitch=0, yaw=0, latitude=latitude, longitude=longitude, altitude=altitude)
    ...
    # 경로를 waypoints로 변환
    def convert_path_to_mavros_waypoints(self, path, altitude=30.0):
        mavros_waypoints = []
